...
class Handyman: UITableViewController {
    ...
    func refreshList(notification: NSNotification){
        ...
        if(handymanArray.count > 0)
        {
            handymanArray.removeAll()
        }
        
        // loop through the array and get all the needed objects for this type
        for var i = 0; i < instanceOfApp.serviceArray.count; ++i {
            
            if(instanceOfApp.serviceArray[i].type == instanceOfApp.handymanTypeString)
            {
                handymanArray.append(instanceOfApp.serviceArray[i])
            }
        }
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        let item = handymanArray[indexPath.row]
